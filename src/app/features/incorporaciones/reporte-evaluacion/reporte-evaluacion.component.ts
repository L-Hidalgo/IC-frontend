...
export class ReporteEvaluacionComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ReporteEvaluacionComponent>
  ) {
    this.loadUser();
  }
  ...
  onSubmit(formReporte: NgForm) {
    if (formReporte.valid) {
      ...
      this.incorporacionesService
        .generarReportEvaluacion(name, fechaInicio, fechaFin)
        .subscribe(
          (data: Blob) => {
            const blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
            const filename = `Reporte de Evaluacion de ${name}.xlsx`;
