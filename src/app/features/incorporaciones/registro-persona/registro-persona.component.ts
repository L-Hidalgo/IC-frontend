...
import { InstitucionesService } from "src/app/core/services/incorporaciones/instituciones.service";
import { PersonasService } from "src/app/core/services/incorporaciones/personas.service";
import { GradosAcademicoService } from "src/app/core/services/incorporaciones/grados-academico.service";
import { AreasFormacionService } from "src/app/core/services/incorporaciones/areas-formacion.service";
import { FormacionesService } from "src/app/core/services/incorporaciones/formaciones.service";
...
export class RegistroPersonaComponent implements OnInit {
  ...
  constructor(
    ...
    this.personaForm = this.formBuilder.group({
      ...
      nombrePersona: [
        dataIncorporacion?.persona?.nombrePersona,
        Validators.required,
      ],
      primerApellidoPersona: [
        dataIncorporacion?.persona?.primerApellidoPersona,
      ],
      segundoApellidoPersona: [
        dataIncorporacion?.persona?.segundoApellidoPersona,
      ],
  ...
  /* --------------------------------------- AREA FORMACION --------------------------------------- */
  loadAreasFormacion() {
    this.areasFormacionService
      .getAll()
      .subscribe((resp: RespuestaLista<AreaFormacion>) => {
  ...
  /* --------------------------------------- GRADO ACADEMICO --------------------------------------- */
  loadGradosAcademico() {
    this.gradosAcademicoService
      .getAll()
      .subscribe((resp: RespuestaLista<GradoAcademico>) => {
  ...
  loadInstituciones() {
    this.institucionesService
      .getAll()
      .subscribe((resp: RespuestaLista<Institucion>) => {
    ...
    if (this.personaForm.valid) {
      const resp = await firstValueFrom(
        this.personasService.createUpdatePersona({
          ...
          primerApellidoPersona: this.personaForm.get("primerApellidoPersona")
            ?.value,
          segundoApellidoPersona: this.personaForm.get("segundoApellidoPersona")
            ?.value,
          ciPersona: this.personaForm.get("ciPersona")?.value,
          expPersona: this.personaForm.get("expPersona")?.value,
          fchNacimientoPersona: this.personaForm.get("fchNacimientoPersona")
            ?.value,
        })
      ...
      if (resp?.objeto) {
        ...
        this.notificationService.showSuccess(
          "Se registro exitosamente a la persona!!!"
        );
      ...
      if (
        ...
        this.personaForm.get("idIncorporacion")?.value
      )
  ...
  async saveDataFormacion(): Promise<void> {
    ...
    if (this.personaForm.get("areaFormacion")?.value) {
      const respAreaFormacion = await firstValueFrom(
        this.areasFormacionService.getByName(
          this.personaForm.get("areaFormacion")?.value
        )
      ).catch((error) => console.log(error));
      if (!!respAreaFormacion?.objeto?.idAreaFormacion) {
        console.log(
          "asignando areaFormacionId a valores:",
          respAreaFormacion?.objeto?.idAreaFormacion
        );
    ...
    if (this.personaForm.get("gradoAcademico")?.value) {
      const respGradoAcademico = await firstValueFrom(
        this.gradosAcademicoService.getByName(
          this.personaForm.get("gradoAcademico")?.value
        )
      ).catch((error) => console.log(error));
      if (!!respGradoAcademico?.objeto?.idGradoAcademico) {
        console.log(
          "asignando gradoAcademicoId a valores:",
          respGradoAcademico?.objeto?.idGradoAcademico
        );
        this.personaForm.patchValue({
    ...
    if (this.personaForm.get("institucion")?.value) {
      const respInstitucion = await firstValueFrom(
        this.institucionesService.getByName(
          this.personaForm.get("institucion")?.value
        )
      ).catch((error) => console.log(error));
      if (!!respInstitucion?.objeto?.idInstitucion) {
        console.log(
          "asignando institucionId a valores:",
          respInstitucion?.objeto?.idInstitucion
        );
        this.personaForm.patchValue({
    ...
    const respFormacion = await firstValueFrom(
      this.formacionesService.createUpdateFormacion({
    ...
    if (respFormacion?.objeto) {
      ...
      this.notificationService.showSuccess(
        "Se registro exitosamente la formacion!!"
      );
  ...
  getDataPersonaById(idPersona: number): void {
    this.personasService.fingById(idPersona).subscribe(
      (resp) => {
        if (!!resp.objeto) {
  ...
  getDataPersonaByCi(): void {
    ...
    this.personasService.fingByCi(ci).subscribe(
      (resp) => {
        if (!!resp.objeto) {
          this.personaForm.patchValue(resp.objeto);
          this.getDataFormacionByPersonaId(resp.objeto.idPersona);
  ...
  getDataFormacionByPersonaId(idPersona: number): void {
    this.formacionesService.buscarPorPersonaId(idPersona).subscribe(
      (resp) => {
